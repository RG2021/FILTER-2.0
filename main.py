...
def scrape():
    ...
    if (c.fetchone()[0]==1):
        ...
        tag_ratings, output_data = final_data_cleaning(new2_df, pos_words, neg_words, final_topics)
        ...
        review_cur.execute("REPLACE INTO reviews (URL, star_1, star_2, star_3, star_4, star_5, total_count, product_name) VALUES (?,?,?,?,?,?,?,?)" ,(table_name, reviews_count[1][1], reviews_count[2][1], reviews_count[3][1], reviews_count[4][1], reviews_count[5][1], total_reviews, product_name))
    ...
    return render_template("newindex.html", reviews=output_data, product_name=product_name, total_reviews=total_reviews, reviews_count=json.dumps(reviews_count), tag_ratings =json.dumps(tag_ratings))
